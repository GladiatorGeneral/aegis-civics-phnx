...
const rawRepresentatives: RawRepresentative[] = [
...
const partyLookup: Record<RawRepresentative["party"], "Democrat" | "Republican"> = {
  D: "Democrat",
  R: "Republican",
};

function makeId(rep: RawRepresentative): string {
  const slug = `${rep.state}-${rep.district}`.replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase();
  return `h-${slug}`;
}

export const houseRepresentatives: GovernmentLeader[] = rawRepresentatives.map((rep) => ({
  id: makeId(rep),
  type: "house",
  name: rep.name,
  title: `Representative for ${rep.state}-${rep.district}`,
  state: rep.state,
  district: rep.district,
  office: rep.office,
  party: partyLookup[rep.party],
  contact: {
    phone: rep.phone,
  },
  committees: rep.committees ? rep.committees.split("|").filter(Boolean) : [],
  metrics: {
    billsSponsored: 0,
    billsPassed: 0,
    voteAttendance: 0,
