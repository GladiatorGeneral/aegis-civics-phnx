...
const partyLookup: Record<RawSenator["party"], "Democrat" | "Republican" | "Independent"> = {
  D: "Democrat",
  R: "Republican",
  I: "Independent",
};

function makeId(senator: RawSenator): string {
  const slug = senator.name.replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase();
  return `s-${senator.state.toLowerCase()}-${slug}`;
}

export const senators: GovernmentLeader[] = rawSenators.map((senator) => ({
  id: makeId(senator),
  type: "senate",
  name: senator.name,
  title: `Senator for ${senator.state}`,
  state: senator.state,
  office: senator.suite,
  party: partyLookup[senator.party],
  contact: {
    phone: senator.phone,
  },
  committees: [],
