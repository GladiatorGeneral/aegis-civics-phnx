...
const rawGovernors: RawGovernor[] = [
...
const partyLookup: Record<RawGovernor["party"], "Democrat" | "Republican"> = {
  D: "Democrat",
  R: "Republican",
};

function makeId(governor: RawGovernor): string {
  const slug = governor.state.replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase();
  return `g-${slug}`;
}

export const governors: GovernmentLeader[] = rawGovernors.map((governor) => ({
  id: makeId(governor),
  type: "governor",
  name: governor.name,
  title: `Governor of ${governor.state}`,
  state: governor.state,
  office: governor.address,
  party: partyLookup[governor.party],
  contact: {
